...
class RosaModel(BaseTuner):
    ...
    **Attributes**:
        ...
        if new_module is None:
            ...
            if isinstance(target_base_layer, torch.nn.Linear):
                ...
                if not kwargs["fan_in_fan_out"]:
                    ...
                    kwargs["fan_in_fan_out"] = rosa_config.fan_in_fan_out = True
            else:
                raise ValueError(
    ...
    # 添加代码（函数forward resize_modules_by_rank_pattern resize_state_dict_by_rank_pattern update_and_allocate）
    ...
    def forward(self, *args, **kwargs):
        ...
        if (getattr(outputs, "loss", None) is not None) and isinstance(outputs.loss, torch.Tensor):
            ...
            for n, p in self.model.named_parameters():
                if ("rosa_A" in n or "rosa_B" in n) and self.trainable_adapter_name in n:
                    if p.shape == torch.Size([0]):
                        with gather_params_ctx(p, fwd_module=self):
                            para_cov = p @ p.T if "rosa_A" in n else p.T @ p
                    else:
                        para_cov = p @ p.T if "rosa_A" in n else p.T @ p
    ...
    def resize_modules_by_rank_pattern(self, rank_pattern, adapter_name):
        ...
        for name, rank_idx in rank_pattern.items():
            ...
            rosa_E_weights = target.rosa_E[adapter_name][rank_idx]
            rosa_A_weights = target.rosa_A[adapter_name][rank_idx]
            rosa_B_weights = target.rosa_B[adapter_name][:, rank_idx]
            ...
            with torch.no_grad():
                if rank > 0:
                    target.rosa_E[adapter_name].copy_(rosa_E_weights)
                    target.rosa_A[adapter_name].copy_(rosa_A_weights)
                    target.rosa_B[adapter_name].copy_(rosa_B_weights)
    ...
    def resize_state_dict_by_rank_pattern(self, rank_pattern, state_dict, adapter_name):
        for name, rank_idx in rank_pattern.items():
            ...
            for layer in ["rosa_E", "rosa_A", "rosa_B"]:
